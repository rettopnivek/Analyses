...
if ( sum( runCode[2:4] ) == 3 ) {
  ...
  Total_trimmed = 100*( 1 - nrow(newDat)/nrow( rawDat ) )
  
  Totals = c( 100*N_too_fast/nrow( rawDat ),
              100*N_too_slow/nrow( rawDat ),
              100*N_unlikely/nrow( rawDat ) )
...
if (saveOutput) {
  setwd( 'Data' )
  save( rawDat, allDat, N, file = 'Priming_Offset.RData' )
}