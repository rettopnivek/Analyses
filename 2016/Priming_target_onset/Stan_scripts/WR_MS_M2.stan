...
parameters {
  ...
  vector<lower=0.0,upper=1.0>[10] theta[Ns]; // Proportion for residual latency
  // Group level
  ...
  real<lower=0.0> theta_alpha[10]; // 1st parameter for beta distribution (Residual latency)
  real<lower=0.0> theta_beta[10]; // 2nd parameter for beta distribution (Residual latency)
}
transformed parameters {
  // Variable declaration
  vector<lower=0.0>[10] tau[Ns]; // Raw residual latency by subject
...
model {
  ...
  for (i in 1:10) {
    theta_alpha ~ normal( Priors[13+i,1], Priors[13+i,2] );
    theta_beta ~ normal( Priors[13+i,3], Priors[13+i,4] );
  }
...
generated quantities {
  ...
  // Create block for local variables
  {
    ...
    for ( ns in 1:Ns ) {
      ...
      // Likelihood
      for ( no in 1:No[ns] ) {
        log_lik[ inc ] = waldraceRL_lpdf( Y[ns,no] | col(param,no) );
        inc = inc + 1;
      }
    }
  }
