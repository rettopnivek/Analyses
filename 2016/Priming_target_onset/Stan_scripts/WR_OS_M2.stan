...
parameters {
  ...
  vector<lower=0.0,upper=1.0>[10] theta; // Proportion for residual latency
}
transformed parameters {
  // Variable declaration
  vector<lower=0.0>[10] tau; // Raw residual latency
...
model {
  ...
  for (i in 1:8) {
    xi[i] ~ normal( Priors[5+i,1], Priors[5+i,2] );
  }
  for (i in 1:10) {
    theta ~ beta( Priors[13+i,1], Priors[13+i,2] );
  }
  ...
  // Likelihood
  for (n in 1:N) {
    summands[n] = waldraceRL_lpdf( Y[n] | col(param,n) );
  }
