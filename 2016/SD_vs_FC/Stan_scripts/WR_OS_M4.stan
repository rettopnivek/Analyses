...
data {
  ...
  real<lower=0.0> min_RT; // Smallest response time for each relevant condition
  matrix[C-8+3,2] Priors; // Matrix of parameters for prior distributions
  vector[8] zIL; // Standardized inverse latencies from nROUSE
}
parameters {
  // S - short, L = long, P - primed choice, B - bias
  real<lower=0.0> kappa_S; 
  real<lower=0.0> kappa_L;
  real kappa_SP;
  real kappa_LP;
  real kappa_B;
  vector<lower=0.0>[8] xi; // Set of drift rates
  real beta[2]; // Regression coefficients
  real<lower=0.0> sigma_xi; // Residual standard deviation for drift rates
...
model {
  ...
  // Priors
  kappa_S ~ normal( Priors[1,1], Priors[1,2] );
  kappa_L ~ normal( Priors[2,1], Priors[2,2] );
  kappa_SP ~ normal( Priors[3,1], Priors[3,2] );
  kappa_LP ~ normal( Priors[4,1], Priors[4,2] );
  kappa_B ~ normal( Priors[5,1], Priors[5,2] );
  //beta[1] ~ normal( Priors[6,1], Priors[6,2] );
  //beta[2] ~ normal( Priors[7,1], Priors[7,2] );
  sigma_xi ~ gamma( Priors[8,1], Priors[8,2] );
  theta ~ beta( Priors[9,1], Priors[9,2] );
  
  // Hierarchy
  xi ~ normal( beta[1] + beta[2]*zIL, sigma_xi );
  
  // Fill in vector of coefficients
  coef[1] = kappa_S; coef[2] = kappa_L;
  coef[3] = kappa_SP; coef[4] = kappa_LP;
  coef[5] = kappa_B;
  coef[ 6:13 ] = xi[ 1:8 ];
  coef[ 14 ] = tau;
