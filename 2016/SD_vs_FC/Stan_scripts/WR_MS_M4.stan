...
data {
  ...
  matrix[ C-8+3, 4 ]Priors; // Matrix of parameters for prior distributions
  vector[8] zIL[Ns]; // Standardized inverse latencies from nROUSE
}
parameters {
  ...
  real beta[Ns,2]; // Regression coefficients
  real epsilon[Ns]; // Residual standard deviation
  ...
  real beta_mu[ 2 ]; // Means for regression coefficients
  real<lower=0.0> beta_sig[ 2 ]; // Standard deviations for regression coefficients
...
transformed parameters {
  ...
  for ( ns in 1:Ns ) tau[ns] = min_RT[ns]*theta[ns];
}
model {
  // Variable declaration
  vector[ C ] coef;
  ...
  for (i in 1:2) {
    beta_mu[i] ~ normal( Priors[5+i,1], Priors[5+i,2] );
    beta_sig[i] ~ gamma( Priors[5+i,3], Priors[5+i,4] );
  }
  epsilon ~ gamma( Priors[8,1], Priors[8,2] );
  theta_alpha ~ normal( Priors[9,1], Priors[9,2] );
  theta_beta ~ normal( Priors[9,3], Priors[9,4] );
  ...
  for ( ns in 1:Ns ) {
    ...
    xi[ns] ~ normal( beta[ns,1] + beta[ns,2]*zIL[ns], epsilon[ns] );
    ...
    beta[ns] ~ normal( beta_mu, beta_sig );
