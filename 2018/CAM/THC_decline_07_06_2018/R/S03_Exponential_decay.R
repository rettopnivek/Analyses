...
run_code = c(
  ...
  F, # 4. Nested cross-validation approach
...
if ( all( run_code[c(2,4)] ) ) {
  ...
  for ( k in 1:K ) {
    ...
    which_sig = which( res1$X.p_value[-val] < .05 )
...
if ( run_code[5] ) {
  ...
  setwd( dat_dir )
  save( bfm, dtbf, param, which_coef, algorithm, cvrts,
...
if ( run_code[6] ) {
  ...
  # Add mean
  lines( plt$Time, plt$R, lwd = 2 )
  # Add estimates
  lines( xa, pred, col = 'blue', lwd = 2 )
