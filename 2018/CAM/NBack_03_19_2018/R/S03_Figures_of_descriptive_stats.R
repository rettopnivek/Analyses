...
# 13) d' means and error bars by subjective high and conditions
...
run_code = c(
  F, # Boxplots of hits over conditions
  ...
  T  # d' means and error bars by subjective high and conditions
)
...
if ( run_code[3] ) {
  ...
  if ( !savePlot ) x11( width = 12, height = 8 )
  ...
  for ( i in 1:nrow( all_cnd ) ) {
    ...
    ")
    if ( all_cnd[i,2] == tm[1] ) v1 = 'Pre'
    if ( all_cnd[i,2] == tm[2] ) v1 = 'Peak'
    if ( all_cnd[i,3] == tm[2] ) v2 = 'Peak'
    if ( all_cnd[i,3] == tm[3] ) v2 = 'Post'
    mtext( paste( v2, '-', v1 ), side = 3, 
           line = 1.5, cex = txtSz )
...
if ( run_code[12] ) {
  ...
  # Meryem's high-low intoxication divisions
  groups = list(
    U50 = as.numeric( 
  ...
  dtbp = data.frame(
    ID = all_ptbp$ID,
    Task = all_ptbp$Task,
    Condition = all_ptbp$Condition,
    Timepoints = all_ptbp$Timepoints,
    SRH = all_ptbp$Self_report_on_high,
    HI = 'Not high',
    H = all_ptbp$H,
    CR = all_ptbp$CR,
    dp = all_ptbp$dp,
  ...
  # dtbp$HI[ dtbp$ID %in% paste( 'FN_0', groups$U50, sep = '' ) ] = 'High'
  
  tmp = dtbp %>% 
    filter( Timepoints == 'T2_Post_drug' ) %>% 
    group_by( ID, Condition ) %>% 
    summarize( HI = unique( SRH ) )
  dtbp$HI[ dtbp$ID %in% tmp$ID[ tmp$HI >= 50 ] ] = 'High'
  
  ct = dtbp %>% 
    group_by( Task, Condition, Timepoints, HI ) %>% 
    summarize( mean_dp = mean( dp ),
               LUI_dp = quantile( 
                 simpleBootstrap( dp )[[1]], .025 ),
               UUI_dp = quantile( 
                 simpleBootstrap( dp )[[1]], .975 ),
               LIQR_dp = quantile( dp, .25 ),
               UIQR_dp = quantile( dp, .75 )
    )
  ...
  pts = rep( rep( c(22,24), each = 3 ), 4 )
  clr = rep( rep( c( 'white', 'black' ), each = 3 ), 4 )
  
  # Create a blank plot
  xl = c( .5, 24.5 ); yl = c( 2, 4.5 )
  blankPlot( xl, yl )
  ...
  axis( 3, c( 6.5, 18.5 ),
        c( '0-back', '2-back' ),
        line = axPos, cex.axis = txtSz, tick = F )
  axis( 1, c( 3.5, 9.5, 15.5, 21.5 ),
        c( 'High < 50', 'High > 50', 'High < 50', 'High > 50' ),
        line = axPos, cex.axis = txtSz, tick = F )
  axis( 2, seq( 2, 4.5, .5 ),
        line = axPos, cex.axis = txtSz, tick = F )
  mtext( "Cognitive performance (d')", 
  ...
  # Order of conditions
  cnd = rbind(
    c( 'NBack_0', 'Placebo', 'Not high' ),
    c( 'NBack_0', 'Drug', 'Not high' ),
    c( 'NBack_0', 'Placebo', 'High' ),
    c( 'NBack_0', 'Drug', 'High' ),
    c( 'NBack_2', 'Placebo', 'Not high' ),
    c( 'NBack_2', 'Drug', 'Not high' ),
    c( 'NBack_2', 'Placebo', 'High' ),
    c( 'NBack_2', 'Drug', 'High' )
  )
  ...
  for ( i in 1:8 ) {
    ...
    sel = ct$Task == cnd[i,1] & ct$Condition == cnd[i,2] & ct$HI == cnd[i,3]
    xa = 1:3 + 3*(i-1)
    
    
    errorBars( xa, error_bars[,sel], 
               lwd = lnSz, length = .05 )
    
    
    # Add labels for timepoints
    if ( i == 1 ) {
      text( 1:3, error_bars[1,sel], c( expression(T[1]),
                                       expression(T[2]),
                                       expression(T[3]) ),
            pos = 1, cex = txtSz )
    }
    
    lines( xa, ya[sel], lwd = lnSz )
    points( xa, ya[sel], pch = pts[xa], bg = clr[xa] )
