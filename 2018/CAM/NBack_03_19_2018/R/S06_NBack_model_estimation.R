...
est_type = c(
  ...
  T, # Estimation
  T, # Bridge sampling for Bayes factor
  T, # LOO-CV
...
if ( run_code[1] ) {
  ...
  # Save results
  save( m1, m1_priors, dtbf, file = 'M1_Nback_posteriors.RData' )
  setwd( proj_dir )
...
if ( run_code[2] ) {
  ...
  # Save results
  save( m2, m2_priors, dtbf, file = 'M2_Nback_posteriors.RData' )
  setwd( proj_dir )
...
if ( run_code[3] ) {
  ...
  # Save results
  save( m3, m3_priors, dtbf, file = 'M3_Nback_posteriors.RData' )
  setwd( proj_dir )
...
###
### 6) Model 4 (Median split)
###

if ( run_code[4] ) {
  
  # Isolate subjects with neural data and with self-reported high
  # either greater than or less than 50
  groups = list(
    U50 = as.numeric( 
  ...
  median_split = list(
    High = unique( 
      cd$Subject[ cd$ID %in% paste( 'FN_0', groups$U50, sep = '' ) ] ),
    Not_high = unique( 
      cd$Subject[ cd$ID %in% paste( 'FN_0', groups$L50, sep = '' ) ] )
  )
  ...
  # Loop over the two groups
  group = c( 'High', 'Not_high' )
  for ( k in 1:length( group ) ) {
    ...
    dtbf = cd %>% 
      select( Task, Condition, Timepoints, Response_type, 
              Subject, Trials, Z, Y, ID )
    dtbf = cbind( dtbf, DM )
    # Isolate group of interest
    dtbf = dtbf[ dtbf$Subject %in% median_split[[k]], ]
    ...
    seed_value = c( 8845, 11230 )
    
    m4 = estimate_sdt( dtbf, glmer_formula,
                       m4_priors, est_type,
                       seed = seed_value[k] )
    
    fname_out = paste( group[k], 'M4_Nback_posteriors.RData', sep = '_' )
    print( fname_out )
    
    # Save results
    save( m4, m4_priors, dtbf, 
          file = fname_out )
